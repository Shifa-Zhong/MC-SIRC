...
def build_manuscript():
    ...
    add_caption(doc, "Figure 3. Forward Monte Carlo uncertainty in nominal river-entry loads before in-stream transport. Red dashed lines show S2 outlet loads; the separation is an inventory–outlet scale discrepancy, not independent evidence of source-coefficient bias.")
    ...
    add_caption(doc, "Figure 4. Spatial identifiability profiles after analytically profiling γ without its former bounds. Curves show relative-error SSE divided by its profile minimum. The negligible ΔR² relative to k = 0 demonstrates that monthly outlet data add virtually no distance information under shared or assumed source calendars.")
